...
void esp_hidd_prf_cb_hdl(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
	...
	case ESP_GATTS_WRITE_EVT: {
		ESP_LOGI(GATTS_TAG, "write evt. handle = %d, value len = %d, value :", param->write.handle, param->write.len);
		break;
	}
	case ESP_GATTS_READ_EVT: {
		ESP_LOGI(GATTS_TAG, "read evt. handle = %d, value :", param->read.handle);
		break;
	}
	case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
		if (param->add_attr_tab.num_handle == BAS_IDX_NB
			...
			//if the battery service started successfully, we can start the HID service
